...
- **Резервна маршрутизація (Fallback Routing)**: У випадку збою основного провайдера (5xx помилки, таймаут або Rate Limits) клієнтський та шлюзовий рівні забезпечують прозоре перенаправлення запитів на резервні моделі (`gemini-3.5-flash` замість `claude-haiku-4-5` та `gpt-5.4-nano` замість `gemini-2.5-flash-lite`), що суттєво покращує High Availability та SLA системи.
